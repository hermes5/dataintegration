 ...
 * <p/>
 * Data sheets are read using a SAX parser to keep the memory footprint relatively small, so this should be able to
 * read enormous workbooks. The styles table and the shared-string table must be kept in memory. The standard POI
 * styles table class is used, but a custom (read-only) class is used for the shared string table because the
 * standard POI SharedStringsTable grows very quickly with the number of unique strings.
 * <p/>
 * Thanks to Eric Smith for a patch that fixes a problem triggered by cells with multiple "t" elements, which is how
 * Excel represents different formats (e.g., one word plain and one word bold).
 * 
 * @author Chris Lott
 */
